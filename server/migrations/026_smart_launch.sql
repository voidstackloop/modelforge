-- SMART App Launch client-role state (server/src/smart-launch/,
-- routes/smart-launch.ts). See docs/SMART_LAUNCH.md.
--
-- Same three-part structure as migration 024 (HL7 ingestion) and every
-- migration since 015/017/018/022 before it: a dedicated
-- `provision_tenant_smart_launch_tables` sub-function (Part 1);
-- provision_tenant_clinical_schema replaced to call it for every *future*
-- organization (Part 2, copied verbatim from migration 024's version of
-- this function except the one new PERFORM line); a backfill for every
-- already-provisioned tenant schema (Part 3).

-- --- Part 1: this domain's own tables ---------------------------------------

CREATE OR REPLACE FUNCTION provision_tenant_smart_launch_tables(schema_name TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    EXECUTE format(
        'CREATE TABLE IF NOT EXISTS %I.smart_trusted_issuers (
    ...
    EXECUTE format(
        'CREATE TABLE IF NOT EXISTS %I.smart_launch_sessions (
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.smart_launch_sessions (requested_by_user_id)', 'idx_' || schema_name || '_smart_launch_sessions_user', schema_name);

    EXECUTE format(
        'CREATE TABLE IF NOT EXISTS %I.smart_launch_tokens (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            issuer TEXT NOT NULL,
            requested_by_user_id UUID NOT NULL,
            scope TEXT NOT NULL,
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.smart_launch_tokens (requested_by_user_id)', 'idx_' || schema_name || '_smart_launch_tokens_user', schema_name);

    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'modelforge_runtime') THEN
        EXECUTE format(
            'GRANT SELECT, INSERT, UPDATE, DELETE ON %I.smart_trusted_issuers, %I.smart_launch_sessions, %I.smart_launch_tokens TO modelforge_runtime',
...
REVOKE ALL ON FUNCTION provision_tenant_smart_launch_tables(TEXT) FROM PUBLIC;

-- --- Part 2: extend provision_tenant_clinical_schema (verbatim copy of
-- migration 024's version, plus one new PERFORM line before RETURN) -------
...
BEGIN
    ...
    -- New this migration.
    PERFORM provision_tenant_smart_launch_tables(schema_name);
...
BEGIN
    FOR org IN SELECT tenant_schema FROM organizations WHERE tenant_schema IS NOT NULL LOOP
        PERFORM provision_tenant_smart_launch_tables(org.tenant_schema);
    END LOOP;
END
$backfill$;
