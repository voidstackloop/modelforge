...
export default function RuntimeManager() {
  ...
  const refresh = useCallback(async () => { if (hasApi) { const [runtimeStatuses, environmentStatuses] = await Promise.all([window.api.localBackends.getStatuses(), window.api.pythonRuntimes.getStatuses()]); setStatuses(runtimeStatuses); setEnvironments(environmentStatuses); } }, [hasApi]);
