...
running, unlocked app — that's narrowed by [session locking](#automatic-session-locking),
...
### Flows (all in `encryption-handlers.ts`, orchestrating `case-encryption.ts` + `patient-cases-store.ts`)
...
| `evidence-sources.json` | Evidence Library entries | No |
...
- **`checkCitations`** (`medical-safety.ts`) exists and is tested but is **not
  yet wired into Clinical Assistant's rendering** — a model's citation markers
  aren't currently cross-checked against Evidence Library entries in the UI.
