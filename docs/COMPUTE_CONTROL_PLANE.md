...
- **True cpuset/processor-affinity pinning and per-workload memory cgroups — not implemented anywhere in this codebase.** No `taskset`/`sched_setaffinity` equivalent, no Linux cgroup, no Windows Job Object, no macOS thread-affinity call. Building this would need either shelling out to platform-specific tools or a native addon per OS, and is a genuinely separate, larger, platform-fragmented undertaking — named here rather than left to be silently assumed done.

## Admin console

`admin-console/src/pages/Compute.tsx` implements fleet/pool capacity, per-node topology/health, cordon/drain/quarantine, active leases/queue, and — as of this pass — a quota-and-fair-share-utilization editor: reserved/burst/weight/borrowing per pool, with a reserved-vs-borrowed utilization bar computed from live active leases (`GET/PUT /compute/pools/:poolId/quota` — the `GET` half didn't exist before this pass; only the pool-creation flow could set a quota blind, with no way to read one back). Still missing from the console, named rather than silently absent: a policy editor with effective-policy preview (would need either a server-side signing endpoint or an offline signing flow, since `canonicalComputePolicyPayload`'s Ed25519 signing today lives in `app/scripts/sign-policy.js`, not anything a browser can safely do client-side), and dedicated audit/policy-version history for this specific domain (the general audit log already captures every compute mutation via `computeLease.allocated`/`computeRequest.submitted`/etc. actions — see Signed resource policies above — just with no compute-specific browsing UI yet).
