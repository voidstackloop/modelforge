...
- **Enrollment stays an admin action**: registering a node (`POST /compute/nodes`) requires the admin-level `compute:manageNodes` permission, which this app never calls on its own behalf. The user copies this device's fingerprint to their organization's compute admin, who registers it and hands back a node id; the user enters that id in the desktop app. From then on the agent only ever calls the agent-scoped endpoints (heartbeat, assignments, lease acknowledge/renew/release).
- **UI: Runtime Manager's "Fleet" tab.** This did not exist for most of this feature's life — `computeAgent:getIdentity`/`getStatus` were built and tested at the IPC-handler level, but never bridged through `preload.ts`, and no frontend page ever called them, so the entire feature was inaccessible to an actual user regardless of how correct the backend was. Found by testing the app as a user would, not by any test (nothing tests preload bridging or checks that a built feature has a reachable UI entry point). Fixed: the fingerprint, a node-id field, and an enable toggle live in Runtime Manager (the compute-control-plane's own designated local node view) and save through the existing generic `settings:save` channel.
